...
export function HomeHeader({ user, balance, signIn, signOut, design }: HomeHeaderProps) {
  ...
  return (
    <header className="flex flex-row items-center justify-between mb-8 md:mb-16 group/nav px-2 sm:px-0" style={getStyle("nav")}>
      {/* Brand / Logo Section */}
      <div className="flex items-center" style={getStyle("brand")}>
      ...
      {/* User Actions / Sign In Section */}
      <div className="flex flex-row items-center justify-center gap-3 sm:gap-4 md:w-auto">
        {user ? (
          <>
            ...
            <div className="flex items-center justify-center gap-2 bg-white shadow-sm border border-slate-200 px-4 py-2 rounded-full hidden sm:flex" dir="ltr" title="الرصيد/المحفظة" style={getStyle("wallet")}>
            ...
            {/* Minimal Mobile Wallet (Just the icon or small balance) */}
            <div className="sm:hidden flex items-center justify-center w-8 h-8 rounded-full bg-white border border-slate-200 shadow-sm" style={getStyle("wallet_mobile")}>
          ...
          <>
            ...
            <button
              onClick={signIn}
              className="md:hidden bg-blue-600 text-white px-5 py-1.5 rounded-[10px] text-sm font-bold shadow-md hover:bg-blue-700 transition-all active:scale-95"
