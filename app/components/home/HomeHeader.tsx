...
export function HomeHeader({ user, balance, signIn, signOut, design }: HomeHeaderProps) {
  ...
  return (
    <header className="flex flex-col xs:flex-row sm:flex-row items-stretch sm:items-center justify-between gap-3 mb-8 md:mb-16 group/nav" style={getStyle("nav")}>
      <div className="flex min-w-0 items-center gap-2 sm:gap-3 bg-white/50 backdrop-blur-md border border-slate-200/50 pl-6 pr-4 py-2 rounded-2xl shadow-sm hover:shadow-md transition-all hover:bg-white hover:-translate-y-0.5" style={getStyle("brand")}>
        ...
        <h1 className="text-base sm:text-xl font-bold tracking-tight text-slate-900 whitespace-nowrap" style={getStyle("brand_text")}>
          متجر <span className="text-blue-600" style={getStyle("accent_text")}>المكافآت</span>
        </h1>
      </div>

      <div className="flex flex-row flex-wrap items-center justify-end gap-2 sm:gap-4 w-full sm:w-auto">
        {user ? (
          <>
          ...
          <button
            onClick={signIn}
            title="تسجيل الدخول"
            className="flex items-center justify-center gap-2 bg-blue-700 text-white shadow-sm hover:shadow-lg hover:shadow-blue-700/20 px-5 sm:px-7 py-2.5 rounded-full font-medium transition-all duration-200 w-full sm:w-auto hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-700 focus-visible:ring-offset-2"
