...
export function HomeClient({ stockMap, layoutOrder = ["hero", "claim", "store"] }: { stockMap: Record<number, number> | null, layoutOrder?: string[] }) {
  ...
  const RenderStore = () => <StoreItems key="store" balance={balance} stockMap={stockMap} />;

  const componentsMap: Record<string, React.ReactNode> = {
    hero: <RenderHero key="hero" />,
    claim: <RenderClaim key="claim" />,
    store: <RenderStore key="store" />,
  };
